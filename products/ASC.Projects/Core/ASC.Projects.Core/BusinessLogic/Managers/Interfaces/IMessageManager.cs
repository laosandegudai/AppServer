...
namespace ASC.Projects.Core.BusinessLogic.Managers.Interfaces
{
    ...
    {
        MessageData GetById(int id, bool checkSecurity);

        MessageData GetById(int id);

        List<MessageData> GetAll();

        List<MessageData> GetProjectMessages(int projectId);

        List<MessageData> GetByFilter();
        ...
        bool CanRead(MessageData message);

        MessageData SaveOrUpdate(MessageData message,
            bool needToNotify,
