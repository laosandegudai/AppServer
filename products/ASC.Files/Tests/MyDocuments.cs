...
namespace ASC.Files.Tests
{
    ...
    public class MyDocuments : BaseFilesTests
    {
        ...
        [OneTimeSetUp]
        public async Task SetUpMyDocuments()
        {
            TestFolder = await FilesControllerHelper.CreateFolder(await GlobalFolderHelper.FolderMy, "TestFolder");
            TestFolderNotEmpty = await FilesControllerHelper.CreateFolder(await GlobalFolderHelper.FolderMy, "TestFolderNotEmpty");
            await FilesControllerHelper.CreateFile(TestFolderNotEmpty.Id, "TestFileToContentInTestFolder", default);
            await FilesControllerHelper.CreateFolder(TestFolderNotEmpty.Id, "TestFolderToContentInTestFolder");
        ...
        {
            var folderWrapper = await FilesControllerHelper.CreateFolder(await GlobalFolderHelper.FolderMy, folderTitle);
            Assert.IsNotNull(folderWrapper);
            Assert.AreEqual(folderTitle, folderWrapper.Title);
            await DeleteFolder(folderWrapper.Id);
        }
        ...
        {
            var folderContentWrapper = await FilesControllerHelper.GetFolder(TestFolder.Id, UserOptions.Id, FilterType.None,  withSubFolders);
        ...
        {
            var folderContentWrapper = await FilesControllerHelper.GetFolder(TestFolderNotEmpty.Id, UserOptions.Id, FilterType.None,  withSubFolders);
            ...
            Assert.AreEqual(filesCountExpected, filesCount);
            Assert.AreEqual(foldersCountExpected, foldersCount);
            await DeleteFolder(TestFolderNotEmpty.Id);
        }
        ...
        {
            var folderWrapper = await FilesControllerHelper .GetFolderInfo(TestFolder.Id);

            Assert.IsNotNull(folderWrapper);
            Assert.AreEqual(folderTitleExpected, folderWrapper.Title);
            Assert.AreEqual(TestFolder.Id, folderWrapper.Id);
            Assert.AreEqual(await GlobalFolderHelper.FolderMy, folderWrapper.ParentId);
        }
        ...
        {
            var folderWrapper = await FilesControllerHelper .RenameFolder(TestFolder.Id, folderTitle);
        ...
        {
            var fileWrapper = await FilesControllerHelper.CreateFile(await GlobalFolderHelper.FolderMy, fileTitle, default);

            Assert.IsNotNull(fileWrapper);
            Assert.AreEqual(fileTitle + ".docx", fileWrapper.Title);
            await DeleteFile(fileWrapper.Id);
        ...
        {
            var fileWrapper = await FilesControllerHelper.GetFileInfo(TestFile.Id);
        ...
        {
            var fileWrapper = await FilesControllerHelper.UpdateFile(
                TestFile.Id,
                fileTitle,
                lastVersion);
        ...
        {
            await FilesControllerHelper.DeleteFile(
                TestFile.Id,
                deleteAfter,
                immediately);
        ...
        public async Task MoveBatchItemsReturnsOperationMove(string json)
        {
            ...
            var statuses = await FilesControllerHelper.MoveBatchItems(batchModel);
        ...
        public async Task CopyBatchItemsReturnsOperationCopy(string json)
        {
            ...
            var statuses = await FilesControllerHelper.CopyBatchItems(batchModel);
