...
namespace ASC.Files.Tests
{
    ...
    class Recent : BaseFilesTests
    {
        ...
        {
            var folderWrapper = Assert.ThrowsAsync<InvalidOperationException>(async () => await FilesControllerHelper.CreateFolder(await GlobalFolderHelper.FolderRecent, folderTitle));
        ...
        {
            var RecentFolder = await FilesControllerHelper.AddToRecent(TestFile.Id);
        ...
        {
            await FilesControllerHelper.SetFileSecurityInfo(TestFile.Id, TestFileShare, notify, message);
            SecurityContext.AuthenticateMe(NewUser.ID);
            var RecentFile = await FilesControllerHelper.AddToRecent(TestFile.Id);
